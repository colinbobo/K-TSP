...
class KTSP:
    ...
    def fit(self, data, labels, ktsp):
        ...
        if self.ktsp == False:
            ...
            if len(self.labels) == self.data.shape[0]:
                ...
                raise Exception(
                    f"Number of samples in data ({self.data.shape[1]}) must match length of labels ({len(self.labels)})")
    ...
    def predict(self, new_samples):
        ...
        if self.ktsp == False or self.optimal_k == 1:
            i = self.best_pair[0]
            j = self.best_pair[1]
